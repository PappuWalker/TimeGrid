<script lang="ts">
	...
	function openDayForm(date: string, entry: Record<string, unknown> | null) {
		openDay = date;
		morningLogin = (entry?.morning_login as string) ?? '';
		morningLogout = (entry?.morning_logout as string) ?? '';
		noonLogin = (entry?.noon_login as string) ?? '';
		noonLogout = (entry?.noon_logout as string) ?? '';
...
<div class="space-y-6 sm:space-y-8">
	...
	<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
		...
		<div class="bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-2xl px-5 sm:px-6 py-3.5 sm:py-4 shadow-lg shadow-indigo-200/50 w-full sm:w-auto">
